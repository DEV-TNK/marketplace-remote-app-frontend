...
const OutsourcedGigsTable = ({ data, header }) => {
      ...
      cell: ({ row }) => {
        if (accessorKey === "header") {
          ...
          const shortenedHeader =
            row.original.gigTitle.length > maxLength
              ? `${row.original.gigTitle.substring(0, maxLength)}...`
              : row.original.gigTitle;
          return (
            <Link
            ...
            >
              <h4 className="mb-1 text-primary-hover">
                {truncateDescription(row.original.serviceType)}
              </h4>
            </Link>
          );
        ...
        } else if (accessorKey === "ongoingServiceType") {
          return (
            <Fragment>
              <Link
                to={`/service/order-summary-single/${row.original.orderSummaryId}`}
                className="text-inherit"
              >
                <span className="mb-1 text-primary-hover">
                  <b>{truncateDescription(row.original.serviceType)}</b>
        ...
        } else if (accessorKey === "serviceProvider") {
          return (
            <Fragment>
              <p className="mb-1 text-primary-hover">
                {row.original.serviceProvider.firstName}{" "}
                {row.original.serviceProvider.lastName}
              </p>
            </Fragment>
          );
            ...
            <Fragment>
              {row.original.paymentStatus === "paid" ? (
                <Button
                  ...
                  onClick={() => markCompleted(row.original.serviceId)}
                >
          ...
          return <span>{formatPrice(row.original.currency, row.original.totalAmountPaid)}</span>;
        } else {
...
export default OutsourcedGigsTable;
